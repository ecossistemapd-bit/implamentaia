...
function DesktopSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  return (
    <aside
      className={`sticky top-0 hidden h-screen shrink-0 border-r border-sidebar-border bg-sidebar lg:flex lg:flex-col transition-[width] duration-300 ease-in-out ${
    ...
    >
      ...
      <div className={`flex items-center py-5 ${collapsed ? "justify-center px-2" : "justify-between px-5"}`}>
        {collapsed ? (
          /* Ícono solo cuando está colapsado */
          <span
            aria-hidden="true"
            className="block h-[26px] w-[26px] shrink-0 bg-foreground"
            style={{
      ...
      <div className={`border-t border-sidebar-border p-2 flex ${collapsed ? "justify-center" : "justify-end pr-3"}`}>
        <button
          onClick={onToggle}
...
function NavList({ collapsed = false }: { collapsed?: boolean }) {
  ...
  return (
    <nav className={`flex-1 overflow-y-auto pb-4 ${collapsed ? "px-1" : "px-3"}`}>
      {sections.map((sec) => (
        <div key={sec.label} className="mb-4">
          {/* Sección label: oculto cuando está colapsado */}
          {!collapsed && (
            <div className="px-3 pb-2 pt-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">
              {sec.label}
            </div>
          )}
          {collapsed && <div className="my-2 h-px bg-gradient-to-r from-transparent via-border to-transparent" />}
            ...
            {sec.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`${item.navClass ?? ""} group flex items-center rounded-lg border-l-2 text-[13px] transition-colors duration-200 ${
                    collapsed
                      ? "justify-center border-transparent px-2 py-2"
                      : "gap-2.5 px-3 py-2"
                  } ${
                    active
...
function UserMenu({ collapsed = false }: { collapsed?: boolean }) {
  ...
  return (
    <div className={`border-t border-sidebar-border py-3 ${collapsed ? "px-1" : "px-3"}`}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          {collapsed ? (
            /* Colapsado: solo el avatar centrado */
            <button
              title={email}
              className="flex w-full justify-center rounded-lg py-1 transition hover:bg-white/[0.08]"
            >
              <Avatar className="h-8 w-8">
                <AvatarImage src={(user?.user_metadata as { avatar_url?: string } | undefined)?.avatar_url} />
                <AvatarFallback className="bg-muted text-primary text-xs">{initial}</AvatarFallback>
              </Avatar>
            </button>
          ) : (
            /* Expandido: email + nombre */
